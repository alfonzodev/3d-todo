...
const TodoItem = ({ todo, completeTodo, deleteTodo }: TodoItemProps) => {
  return (
    <div className="flex mb-3">
      <label className="grow flex items-center gap-4 border rounded-md py-2 text-3xl px-6 border-gray-400 bg-white hover:bg-slate-50 cursor-pointer overflow-x-scroll whitespace-nowrap">
        <input
          type="checkbox"
          className="scale-150"
      ...
      <button className="grow-0 p-5" onClick={() => deleteTodo(todo.id)}>
        <RiDeleteBinLine className="text-4xl hover:text-red-600" />
      </button>
    </div>
  );
};
